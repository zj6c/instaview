...
export default function AppPage() {
    ...
    try {
      ...
      await Promise.all(Array.from(files).map(async f => {
        try {
          ...
          // Store by full name and base name
          entries[f.name] = url
          entries[f.name.replace(/\.[^.]+$/,'')] = url
          // Also store by numeric ID if filename is numeric
          const numMatch = f.name.match(/^(\d+)/)
          if (numMatch) entries[numMatch[1]] = url
        } catch(e) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#090909]"
        ...
        {activeConv ? (
          <>
            <div className="flex items-center gap-3 px-4 py-2.5 glass border-b border-ig-border flex-shrink-0">
              ...
              <button onClick={()=>mediaRef.current?.click()} className="text-ig-muted hover:text-ig-text p-1.5 rounded-lg hover:bg-white/5 transition-all" title="رفع وسائط">
                <ImgIcon size={15}/>
              </button>
              <button onClick={()=>htmlRef.current?.click()} className="text-ig-muted hover:text-ig-text p-1.5 rounded-lg hover:bg-white/5 transition-all" title="إضافة ملف">
                <Plus size={15}/>
              </button>
            </div>
      ...
      <input ref={htmlRef}  type="file" accept=".html,.htm" multiple className="hidden" onChange={e=>{onHtmlFiles(e.target.files);e.target.value=''}}/>
      <input ref={mediaRef} type="file" accept="image/*,video/*,audio/*" multiple className="hidden" onChange={e=>{onMediaFiles(e.target.files);e.target.value=''}}/>
    </div>
  )
}
